...
__irq void eth_rx_isr()
{
	...
	while(ETH_MII[ETH_MII_GENERAL_STATUS] & 0x7E)	// Loop around all packets in the Ethernet buffer
	{
		...
		if(IP(rx_frame))							// if the frame received is validated as an IP packet
		{
			...
			{	
			    if(	SPINN_INSTRCTN(rx_frame) != HELLO ) {	// CP: DHCP Bug Fix - shouldn't allocate DIP/DMAC if another board's hello message !!
			    						//           feature? To stop lots of hello packets from stray boards?
			...
			{		
				if ((((rx_frame[7] & 0xFFFF) << 16) | ((rx_frame[8] & 0xFFFF0000)>>16)) == sourceIPAddress && sourceIPAddress!=0) 
						// CP: DHCP fix. Don't process if not for my unicast IP or if IP address unallocated by DHCP yet (0.0.0.0)
					handlePingRequest();	// turn around a ping reply to the request
					
			#ifdef DHCP
			} else if (UDP(rx_frame) && !dhcp_learned && BOOTP(rx_frame)) {							// BOOTP is the parent protocol of DHCP
				dhcp_learned = dhcp_pkt((eth_frame_t *) rx_frame, FRAME_LEN(rx_frame));
			#endif // DHCP
			} 
		}
		else if(ARP(rx_frame) && (((rx_frame[9] << 16) | (rx_frame[10] >> 16)) == sourceIPAddress) && sourceIPAddress!=0)
		{			// CP: DHCP fix. Should not reply if IP address unallocated by DHCP yet (0.0.0.0) 
			handleArpRequest();
		}
...
__irq void timer_isr()
{
	...
	if(PHY_PRESENT) {
		if(sourceIPAddress!=0) if (!(time % HELLO_FREQUENCY)) send_hello_frame();
			// send a hello packet at the prescribed interval only if we have an IP address
			// if 0 we wait on DHCP allocation (below)
		#ifdef DHCP
		timereddhcpoperations();			// call operations required if DHCP is in use
		#endif // DHCP
