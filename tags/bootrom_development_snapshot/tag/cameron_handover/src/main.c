...
unsigned int arbitrate_monitor()
{
	...
	{
		printf("A5\n");	//!!! TLM !!!
		//TODO arbitration bit might not be free if monitor has died, so we can't get new monitor status 
		if (SYS_CTRL[SYS_CTRL_ARBIT_BASE + processor_ID])						// arbitrate for monitor 
		{
			printf("A6\n");	//!!! TLM !!!
			SYS_CTRL[SYS_CTRL_CPU_DISABLE] = *PAST_MONITORS;					// if we are elected monitor, disable previous monitors (note security code already in past_monitors) TODO don't disable clock, just switch off interrupts and send to sleep
			printf("A7\n");	//!!! TLM !!!
...
//	pointer ETH_MII = (uint*) ETH_MII_BASE;
	...
	{
		SYS_CTRL[GPIO_DIR]&=0xFFFFFF3C;    // set up GPIO direction outbound for LEDs TODO check that the values that these are being set to are correct for SpiNNaker2 LEDS
		SYS_CTRL[GPIO_CLR]=0xC3; // Turn off all LEDs	TODO what colours these should be? we've only got 2 per chip!
...
#ifdef SPINNAKER2 //TODO this is TLM testing, remove before final final build!!!
{	
