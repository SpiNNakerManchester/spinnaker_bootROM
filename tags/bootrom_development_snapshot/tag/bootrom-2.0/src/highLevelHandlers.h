...
//		responseOutDue=1;  // CP - this is a bug, as there may be no responses to send!  Deprecated 19th Jan 2010.
	if(statusReportDue==1)
	{		
		if( (currentTime >= (reportStatusTime+WAIT_FOR_REPORTSTATUSOUT)) )// && !ackFrameDue) 
		{
			//printf("Status Report - time to send! AckFrameDue: 0x%x. Mode: 0x%x\n",ackFrameDue,opMode); // debug
			if(opMode==IDLE)
	...
	if (EthLinkUp == 0)
	  {
	    ...
	    EthLinkUp = 1; 
            ...
            if(macDestAddressConfigured==0) opMode=HELLO;  // CP 15th Jan, if we haven't see the host (dest MAC not configured), then Eth is up and we need to send hellos
   ...
   if(rxFrameBase[9]>>16 == 54321) {
	...
	switch(instruction)
	{
		...
		case ACK: //ack frame
			...
			{
				printf("More Status Reports Due\n");
				if(opMode==IDLE)
					opMode=SEND_STATUS_REPORT;
