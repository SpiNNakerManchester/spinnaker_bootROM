...
void initMacFromSerialRom()
{
	sourceMACAddressLS=0x48000001;
	sourceMACAddressHS=0xACDE;
}
...
void initIPAddressFromSerialRom()
{
	sourceIPAddress=0xC0A80002;	//192.168.0.2
	desinationIPAddress=0xC0A80001;	//192.168.0.1
}
*/
/*******************************************************************
 * 
...
void initFrameHeaders(unsigned int spinnakerdatalength)
{
    ...
    txFrameBase[4]=((20+8+18+spinnakerdatalength) << 16);                //Total IP Datagram Length in Bytes = 20(IP)+8(UDP)+18(SpiNN)+SpiNNakerLength(Extras)    + Identification (for fragments) = 0                                           
    ...
    txFrameBase[9]=(54321 << 16)  | (8+18+spinnakerdatalength);                //Destination UDP port = 54321 + UDP length = 8(UDP)+18(SpiNN)+SpiNNakerLength(Extras)                                           
...
int formatAndSendFrame(unsigned int instruction, unsigned int opton1, unsigned int option2, unsigned int option3)
{
	...
	txFrameBase[dataLengthAt/4]=(txFrameBase[dataLengthAt/4] & 0xFFFF0000) | (option2 & 0xFFFF); //length to go at byte 42, 43
...
void sendHelloFrame()  
{
	int length=formatAndSendFrame(HELLO, 1, 0, 0);   //instruction, option1=sub-instruction (system-wide configuration done), option2=size
}
/*******************************************************************
 * 
 * 
