	         	...
	         	case HELLO:
	         	{
	         		...
	         		if(EthLinkUp)
	         		{
						...
						hostConnectedChip=1;
	         			...
	         			else
	         			{
	         				if(ackFrameDue)
	         				{
	         					if(currentTime >= helloTime+100) //if ack does not come in 100 milliseconds, send hello again
	         					{
	         						frameSent=0;
	         					}
	         					else
	   			...
	   			case FLOOD_FILL:								 	//floodfill data
	   			{
					...
					case SEND_STATUS_REPORT:
						...
						if((spinnakerSystemSize/8) > 1420) //more than one frame size
						{
							if(statusReportIndex < spinnakerSystemSize/32 )   //if status sent so far (statusReportIndex) are less than the due (spinnakerSystemSize)
							{
								if(!ackFrameDue)
								{
									...
									formatAndSendFrame(REPORT_STATUS, 0, count, 0);  //chip status is stored in the statusBuffer as a key:value pair, requiring 1 word for the chip address and 1 for its status totol=no_of_status_received*2*4
						...
						{
							for(count=0; count < spinnakerSystemSize/8; count++)          //to make it 60 bytes equivallent
							{
