...
/*		// CP:  Now detecting SDRAM size ourselves 14thMay2010
#ifdef QUIMONDA						// on the testboard with the testchip Quimonda parts are 256Mb / 32MB
 #define SD_RAM_SIZE				0x02000000
#endif
#ifndef QUIMONDA					// either the Micron Testboard part, or on the main chip we have a 1Gb / 128MB part
 #define SD_RAM_SIZE				0x08000000
#endif
*/
...
/* ------------------------------------ PL340 register offsets -----------------------------------*/
#define MC_CMD  					(0x04 / 4)
#define MC_DIRC 					(0x08 / 4)
#define MC_MCFG						(0x0C / 4)
#define	MC_REFP						(0x10 / 4)
#define MC_CASL						(0x14 / 4)
#define MC_CCFG0					(0x200/ 4)
#define DLL_CONFIG0					(0x304/ 4)
#define DLL_CONFIG1					(0x308/ 4)
...
#define PL340_REFRESH_PRD			(0x0010 / sizeof(int))
...
/* --------------------------------- STATUS VARIABLES ----------------------------------*/
//extern unsigned int phy_present; 			//  now a macro =1 implies ethernet phy exists established, 0 no PHY chip
//extern unsigned int monitor;			// Removed and replaced by macro with DMA CTRL and SYS CTRL register references
//extern unsigned int processor_ID; 		// now a macro. Processor ID on chip
...
// CP section for LED support of test board
//#define LED_FLASH_INTERVAL			0x100
