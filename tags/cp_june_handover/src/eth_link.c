...
void handleArpRequest()
{
	...
	sendFrame(42);  // send ARP reply on it's way, 42byte length frame (padded later). If sending fails then ignore, it's not critical.
}
...
void handlePingRequest()
{
	...
	sendFrame(size);  // send it on it's way.  If sending fails then ignore, it's not critical.
}
...
void initFrameHeaders(unsigned int spinnakerdatalength)
{
    ...
    tx_frame[0]=(destinationMACAddressHS & 0xFFFF)<<16 | (destinationMACAddressLS & 0xFFFF0000)>>16; //4 MSBytes of  Dest MAC address
    tx_frame[1]=(destinationMACAddressLS & 0xFFFF)<<16 | (sourceMACAddressHS & 0xFFFF);		 //2 LSBytes of DMAC, first 2 MSB of SMAC
    ...
    tx_frame[4]=( ((20+8+18+spinnakerdatalength) << 16) | EthPacketID++ );  //IP Sz = 20(IP)+8(UDP)+18(SpiNN)+SpiNPayload(Extras),PktID                                          
    tx_frame[5]=IP_FRAG_FLAGS_TTL_PROT;										//Frag, flags, offset, TTL, IP Protocol UDP
    tx_frame[6]=0x0|(sourceIPAddress & 0xFFFF0000)>>16;						//IP Hdr Chk (init 0 - see later) | 2 octets SrcIP address   
    tx_frame[7]=(sourceIPAddress  & 0xFFFF)<<16 | (destinationIPAddress & 0xFFFF0000)>>16;	//last 2 octets SrcIP, 1st two of DstIP
    tx_frame[8]=(destinationIPAddress  & 0xFFFF)<<16 | SPINN_PORT;			//last 2 octets of DstIP, Source UDP port                                       
    tx_frame[9]=((uint)SPINN_PORT << 16)  | (8+18+spinnakerdatalength);			//Dest UDP port, UDP Length (UDP Headers+SpiNN+Extras)                                           
    ...
    for(count=3; count<8; count++) sum+= ( (tx_frame[count] & 0x0000FFFF) + ((tx_frame[count+1] & 0xFFFF0000)>>16) );
    ...
    tx_frame[6]=(count << 16) | tx_frame[6];  //insert the ip checksum in the correct position in the header
 }
...
int formatAndSendFrame(unsigned int instruction, unsigned int option1, unsigned int option2, unsigned int option3)
{
	...
	tx_frame[spinnakerInstructionAt/4]=instruction;		//instruction to go at instruction field
	tx_frame[instructionOption1At/4]=option1;			//option1 to go at option1
	tx_frame[instructionOption2At/4]=option2;			//to go at option2
	tx_frame[instructionOption3At/4]=option3;			//to go at option3

	sendFrame(length);									//whack the frame out on the Ethernet
	return length;   									//length of frame transmitted for those interested
}
...
void send_itubotron_message()
{
	pointer CHIP_IDENTIFIER = (uint*) CHIP_ID;
	uint i;

	for(i=0;i<itubotroncount;i++) tx_frame[(spinnakerDataAt/4)+i]=itubotrondata[i];
	formatAndSendFrame(ITUBOTRON, ITUBE_CMD<<16|itubotrondataoverflow, itubotroncount*4, (unsigned int) *CHIP_IDENTIFIER);
		// this sends itubotron frame, the command 4, count of any overflowed chars, length of frame, and my ID (sender of Eth messg)
	itubotroncount=0;
	itubotrondataoverflow=0;		// reset counter variables once frame has been sent
} 
