...
#define INT_PACKET_DELAY_US	50000		// this is the delay between packets being sent (in micro seconds). Eg 50000 = 50ms (20 per second)
#define IMAGE_REPEAT_DELAY_S	2		// this is how long we wait to begin transmitting the image in full once again (in seconds)
...
int main(int argc, char *argv[]) {
	...
	struct timeval tv;
	unsigned long last_usec, last_sec, current_usec, current_sec;
...
//	time_t current;
	...
	if (romimage==NULL) {
		...
		printf("  %d Blocks of size; %d Bytes.  Leftovers in final block: %d Bytes.\n",data_block_count,MAXBLOCKSIZE*4,data_leftovers_size);  
...
*/		
			...
			} while ( ((current_sec*1000000)+current_usec) < ((last_sec*1000000)+last_usec+INT_PACKET_DELAY_US) );
				...
				if(block_id==data_block_count) {
					...
					workingpacket.option2=htonl((block_data_len*4)+remaining_bytes);
				 	if (remaining_bytes!=0) {  // if not a word aligned number of bytes in last packet
						...
						printf("RemainingBytes: 0x%x. Additional bytes: 0x%x ...\n",remaining_bytes,workingpacket.array[block_data_len]);
...
*/
			...
			do {
				...
				current_usec = tv.tv_usec;
				current_sec = tv.tv_sec; 
			} while ( ((current_sec*1000000)+current_usec) < (((IMAGE_REPEAT_DELAY_S+last_sec)*1000000)+last_usec) );
