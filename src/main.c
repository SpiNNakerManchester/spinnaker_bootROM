...
unsigned int arbitrate_monitor()
{
	...
	else
	{
		...
		{
			*PAST_MONITORS |= pid;
			SYS_CTRL[SYS_CTRL_MISC_CTRL] |= 0x2; // CP: Let the non DHCP processors go (ensures boot proc=monitor) 
			return 1;
		}
	}
...
void boot_processor()
{
	...
	if(arbitrate_monitor())
	{
		...
		if (ETH_PARAMS[0]&0x00000080) {						// CP DHCP 10thNov2011. Board type is SPINN3 with 2 LEDs, (top bit of LSByte is 1)
			SYS_CTRL[SYS_CTRL_GPIO_DIR] &= ~LEDSSPINN3;
			SYS_CTRL[SYS_CTRL_GPIO_SET] = LEDSSPINN3;
		} else {								// CP DHCP 10thNov2011. Board type is SPINN2 with 4 LEDs
			SYS_CTRL[SYS_CTRL_GPIO_DIR] &= ~LEDSSPINN2;
			SYS_CTRL[SYS_CTRL_GPIO_SET] = LEDSSPINN2;
		}	
		
		SYS_CTRL[SYS_CTRL_GPIO_CLR] = LED_0; // Begin wiggling the green LED
