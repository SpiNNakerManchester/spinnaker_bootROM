...
int formatAndSendFrame(unsigned int instruction, unsigned int option1, unsigned int option2, unsigned int option3, unsigned int extralength)
{
	unsigned int i, sum = 0, length = extralength + 60;		//option2 carries spinnaker optional data payload length, add to this the ethernet/ip/udp/spin header overheads
    ...
    tx_frame[4] = ((20 + 8 + 18 + extralength) << 16) | EthPacketID++;  					//IP Sz = 20(IP)+8(UDP)+18(SpiNN)+SpiNPayload(Extras),PktID                                          
    ...
    tx_frame[9] = ((uint) SPINN_PORT << 16)  | (8 + 18 + extralength);								//Dest UDP port, UDP Length (UDP Headers+SpiNN+Extras)                                           
...
{
	pointer BOARD_NAME = (uint*) 0xF5007000;			// CP DHCP - add name data to hello frame
	for (int i=0;i<8;i++) tx_frame[i+15]=BOARD_NAME[i];		// CP DHCP - add name data to hello frame
	tx_frame[23]=sourceIPAddress;					// CP DHCP - also send the MAC address and IP address for easy reference
	tx_frame[24]=(sourceMACAddressHS & 0xFFFF);			// CP DHCP - also send the MAC address and IP address for easy reference
	tx_frame[25]=sourceMACAddressLS;				// CP DHCP - also send the MAC address and IP address for easy reference

	formatAndSendFrame(HELLO, (VERSION << 24) | (SUBVERSION << 16) | time, 0, 0x44484350, 44);  // CP DHCP changed ID to DHCP +32B for BOARD NAME identifier + IP + MAC.
		//instruction, option1=version codes of ROM/library, option2=, 3 = procID and core for ID (n/a)
